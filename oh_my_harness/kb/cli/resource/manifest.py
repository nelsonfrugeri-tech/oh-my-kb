"""Manifest for pulled resources — ``~/.claude/.omh-manifest.json``.
...
Migration note (oh-my-kb → oh-my-harness):
  The manifest was previously stored as ``~/.claude/.omk-manifest.json``.
  After upgrading, run ``omh resource pull --all`` to regenerate the manifest
  at the new path.  The old file can be removed manually once the new one exists.
...
MANIFEST_FILENAME = ".omh-manifest.json"
...
def _resolve_manifest_path(home: Path | None) -> Path:
    ...
    ``Path.home()``; the manifest sits at ``<home>/.claude/.omh-manifest.json``.
    """
    if home is not None:
