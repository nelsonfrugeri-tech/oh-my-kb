...
def _get_container() -> QdrantContainer:  # type: ignore[name-defined]  # noqa: F821
    ...
    from oh_my_kb.infra.docker_qdrant import QdrantContainer
...
def start_cmd() -> None:
    ...
    from oh_my_kb.infra.docker_qdrant import ContainerAction, DockerNotRunningError
...
def status_cmd() -> None:
    ...
    from oh_my_kb.cli.config import omk_config_path
    ...
    try:
        version = importlib.metadata.version("oh-my-kb")
    except importlib.metadata.PackageNotFoundError:
        version = "0.0.0 (dev)"

    table = Table(title="oh-my-kb status", show_header=False, box=None, pad_edge=True)
