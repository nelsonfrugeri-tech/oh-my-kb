...
uses (:func:`oh_my_kb.services.paths.default_notes_root_for`). Future
...
Imports come from :mod:`oh_my_kb.services.paths`, the neutral shared layer,
...
from oh_my_kb.services.paths import DATA_ROOT_ENV, default_notes_root_for
...
def get_active_notes_root(universe: str | None = None) -> Path:
    ...
    **data root** (parent of all universes), not as a direct per-universe
    ...
    ``KB_NOTES_ROOT`` unset, universe ``eng``  →  ``~/oh-my-kb/eng``
    """
